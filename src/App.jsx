...
import { fetchPokemonBundle, fetchPokemonBundleByDexId, randPokemonId, toID } from './pokeapi.js';
import { defaultSave, loadSave, saveSave } from './storage.js';
import { getEvolutionOptions } from './evolution.js';
import { getRandomDexId } from './dexLocal.js';
...
// dexNum = dexNum ?? lookupDexNum(speciesId);
  ...
  // Sprite resolution (Showdown first, fallback) - preserve shiny through evolution
  const primarySprite = mon.shiny ? (evolvedBundle.shinySpriteUrl || evolvedBundle.showdownSprite) : evolvedBundle.showdownSprite;
  const spriteUrlResolved = await resolveSpriteUrl(
    primarySprite,
    evolvedBundle.fallbackSprite
  );
...
// Build evolved record while keeping buff/rarity
  ...
  async function spawn() {
    ...
    try {
      // Spawn either a random national-dex number or a random PS dex id (forms can spawn distinctly)
      const useDexId = Math.random() < 0.35; // 35% chance to spawn via PS dex id (forms)
      const id = useDexId ? getRandomDexId() : randPokemonId();
      const bundle = useDexId ? await fetchPokemonBundleByDexId(id) : await fetchPokemonBundle(id);
      ...
      setWild({
        ...
        buff,
        shiny: isShiny,
        spriteUrl: isShiny ? (bundle.shinySpriteUrl || bundle.showdownSprite) : bundle.showdownSprite,
        fallbackSprite: isShiny ? (bundle.fallbackShinySprite || bundle.fallbackSprite) : bundle.fallbackSprite,
      });
...
if (isShiny) {
  ...
  async function throwBall(ballKey) {
    ...
    // Resolve sprite (Showdown first; fallback to PokéAPI official artwork)
    const isShiny = !!wild.shiny;
    const spriteUrlResolved = await resolveSpriteUrl(wild.spriteUrl, wild.fallbackSprite);
        ...
        {teamMons.length === 0 ? (
          ...
          <div className="teamList">
            {teamMons.map((m) => (
              ...
              >
                <img className="teamSprite" src={m.spriteUrl} alt={m.name} />
            ...
            <>
              <div className="mobileActiveMeta">
                ...
                <span className="mobileActiveSub">
                  {monMovesUsedCount}/4 • KO {Math.round(nextKoChance * 100)}%
                </span>
              </div>
              ...
              <div className="mobileMovesGrid">
                {(activeMon.moves ?? []).slice(0, 4).map((mv, idx) => (
                  <button
                    ...
                    disabled={usedMoves[idx] || stage !== 'ready'}
        ...
        {!activeMon ? (
          ...
          <>
            <div className="activeMonCard">
              <img className="activeMonSprite" src={activeMon.spriteUrl} alt={activeMon.name} />
              <div>
            ...
            <div className="teamCircles">
              {teamMons.map((m) => (
                <button
                  ...
                  type="button"
                >
                  <img className="teamCircleSprite" src={m.spriteUrl} alt={m.name} />
                </button>
              ))}
            </div>
          )}
...
async function resolveSpriteUrl(primaryUrl, fallbackUrl) {
  const ok = await canLoad(primaryUrl);
