...
function bumpDexCaughtFromAny(anyIdOrNum, isShiny, isDelta, rarityKey) {
  ...
  // Mark encounter as actually shown (only when it hits the encounter square)
  function markEncounterShown(w) {
    if (!w) return;
    bumpSeen(w.rarity, !!w.shiny, !!w.isDelta);
    bumpDexSeenFromAny(w.dexId ?? w.name ?? w.num ?? w.id, !!w.shiny, !!w.isDelta);
  }
